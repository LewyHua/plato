...
import (
	...
	"github.com/lewyhua/plato/state/rpc/client"
...
func ackMsgHandler(cmdCtx *service.CmdContext, msgCmd *message.MsgCmd) {
	log.Println("ackMsgHandler", cmdCtx.ConnID, string(msgCmd.Payload))
...
func upMsgHandler(cmdCtx *service.CmdContext, msgCmd *message.MsgCmd) {
	log.Println("upMsgHandler", cmdCtx.ConnID, string(msgCmd.Payload))
	upMsg := &message.UPMsg{}
...
func reConnMsgHandler(cmdCtx *service.CmdContext, msgCmd *message.MsgCmd) {
	log.Println("reConnMsgHandler", cmdCtx.ConnID, string(msgCmd.Payload))
...
func hearbeatMsgHandler(cmdCtx *service.CmdContext, msgCmd *message.MsgCmd) {
	log.Println("hearbeatMsgHandler", cmdCtx.ConnID, string(msgCmd.Payload))
...
func loginMsgHandler(cmdCtx *service.CmdContext, msgCmd *message.MsgCmd) {
	log.Println("loginMsgHandler", cmdCtx.ConnID, string(msgCmd.Payload))
...
func sendACKMsg(ackType message.CmdType, connID, clientID uint64, code uint32, msg string) {
	log.Println("sendACKMsg", connID, code, msg)
	ackMsg := &message.ACKMsg{}
	ackMsg.Code = code
	ackMsg.Msg = msg
